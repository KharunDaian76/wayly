...
**Unchanged card/panel behavior:** **View details** (drawer), **Copy reference** (inside drawer), chat, mock payment, proof, dispute, order-linked notification focus/highlight + drawer open, converted-order source badges, post-accept auto-refresh from availability-request accept, **Sender Published Orders** panel (create/publish/cancel + panel states — see **Sender Published Orders panel states**), and **Wayler OPEN feed** accept flow (see **Wayler open orders feed panel states**).
