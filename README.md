...
wayly/
├─ apps/
│  ├─ web/                 # Next.js: landing + PWA app + admin Operations Center on /app
...
| Admin Operations Center (six panels on `/app`; read-only Orders/Users/Payments/System Health; `GET /api/v1/admin/*`)                                                                                                                        | Complete (foundation)                                                                                                                                                     |
| Admin KYC approve/reject v1 (`POST /api/v1/admin/kyc-verifications/:id/approve` + `/reject`; manual review only)                                                                                                                            | Complete (v1)                                                                                                                                                             |
| Admin dispute resolution v1 (`POST /api/v1/admin/disputes/:id/resolve`; metadata only — no payment execution)                                                                                                                               | Complete (v1)                                                                                                                                                             |
| Admin audit log foundation v1 (`AdminAuditLog` + `GET /api/v1/admin/audit-logs`; append-only KYC/dispute audit)                                                                                                                             | Complete (v1)                                                                                                                                                             |
...
| Admin/arbitrator panel + dispute resolution v1 (metadata) + payment hold on dispute                                                                                                                                                         | Admin mutations v1 complete for KYC + dispute metadata — see [Admin Operations Center](docs/admin-operations-center.md); payment execution on dispute outcome not started |
...
- **Admin / manual review v1** — operator KYC approve/reject on Operations Center (`POST /api/v1/admin/kyc-verifications/:id/approve|reject`); manual/mock only — no real provider integration yet
...
Wayly is building a **safety structure for order problems** before real payments go live. The current implementation includes **database schema**, **shared types**, **`DisputesModule` API + SDK**, **Sender/Wayler Accepted dispute UI** on `/app`, **in-app dispute notifications** to the other participant (open, message, evidence — via `SYSTEM` until dedicated types exist), an **Admin Disputes queue** with **manual resolution v1** for `ADMIN` / `ARBITRATOR` on the Operations Center (`POST /api/v1/admin/disputes/:id/resolve` — metadata only; see [Admin Operations Center](docs/admin-operations-center.md)), and **no payment hold/refund integration or file upload yet.**
...
Other participant notified in-app (bell/dropdown via SYSTEM + relatedOrderId)
        ↓
Admin/Arbitrator: manual resolution v1 on Operations Center (note + optional outcome metadata)
        ↓
Payment/escrow handling depends on dispute result (future — not executed by admin resolve v1)
```
...
| `DisputesModule` API + Swagger                                       | Assign arbitrator workflow                  |
...
| Admin dispute resolve v1 (`POST /api/v1/admin/disputes/:id/resolve`) | Dispute payment execution on resolve        |
| i18n `app.disputes.*` (8 locales)                                    |                                             |

### Current limitations

- **Admin dispute resolve is metadata-only** — `POST /api/v1/admin/disputes/:id/resolve` sets status/note/outcome on existing fields; **no refund, release, or payment side effects**
- **No assign-arbitrator workflow** — `assignedArbitratorId` not set via admin UI/API yet
...
- **No resolution notifications** — resolve outcomes not dispatched to parties yet
...
- **Admin/arbitrator dashboard** — review queue with **resolve v1** (metadata); assign arbitrator still future
- **Assign arbitrator** — set `assignedArbitratorId`; queue routing for `ADMIN`/`ARBITRATOR` roles
- **Dispute payment execution on resolve** — execute refund/release/capture/payout from `DisputeResolution` outcome on `PaymentIntent` / ledger
...
| Users & Trust/Safety  | `GET /api/v1/admin/users`             | — (read-only)                          |
...
**Safety:** KYC approve/reject and dispute resolve are **controlled metadata mutations** on existing schema fields — no refund, release, capture, payout, order cancel/reassign, or user moderation. **Append-only audit log** records KYC approve/reject and dispute resolve (best-effort after success). Orders, Users, and Payments panels remain read-only. No secrets in admin responses.
...
- **M12 — Admin Operations Center:** **`AdminOperationsDashboard`** on `/app` for **`ADMIN` / `ARBITRATOR` only**; six panels — **Disputes** (list + **resolve v1**), **KYC** (list + **approve/reject v1**), **Orders**, **Users & Trust/Safety**, **Payments & Escrow**, **System Health & Audit** (health snapshot + **recent admin actions** from audit log); SDK `api.admin.*`; loading/error/retry/empty/list UX; **append-only `AdminAuditLog`** for KYC/dispute mutations (best-effort writes). ✅ (foundation + KYC + dispute + audit log v1). Next: payment refund/release/payout, user moderation, strict transactional audit, real provider admin hooks, assign arbitrator, admin realtime, dispute payment execution. See [Admin Operations Center](docs/admin-operations-center.md).
