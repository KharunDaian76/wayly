...
| Accepted Order Details drawer (View details; Sender + Wayler; frontend-only; **Delivery progress timeline**; **loading/error/retry**; **Copy reference**) | Complete (M4)                   |
...
| Sender Accepted panel (`GET /orders/mine` ACCEPTED/IN_TRANSIT/DELIVERED) + **loading/empty/error/retry/refresh**                                          | Complete      |
| Accepted Order Details drawer (View details; Sender + Wayler; frontend-only; **Delivery progress timeline**; **loading/error/retry**; **Copy reference**) | Complete      |
...
**Unchanged feed behavior:** **Accept** posted OPEN order (`handleAcceptOrder` → `api.orders.accept`), own-order block (`senderCannotAcceptOwn`), KYC gate, **daily Wayler access gating on accept**, filters/sort/map previews, accept success/error alerts, **Accepted delivery requests panel refresh** after accept (`loadFeedOrders()` + `loadAcceptedOrders()`), **Accepted Order Details drawer**, and **order-linked notification click** (focus accepted card + open drawer via **`DeliveryOrder.id`**) — all unchanged.
...
**Unchanged card/panel behavior:** **View details** (drawer), **Copy reference** (inside drawer), chat, mock payment, proof, dispute, order-linked notification focus/highlight + drawer open, converted-order source badges, and post-accept auto-refresh from availability-request accept.
...
**Cross-panel (notifications + drawer):**
...
6. **Open details drawer** — when the order is found, opens the **Accepted Order Details drawer** (including **Delivery progress** timeline, **loading/error/retry** with accepted-row fallback, and **Copy reference**) for that same **`DeliveryOrder.id`** (same component/state as **View details** on the card). Drawer enriches via existing **`GET /orders/:id`** (`DeliveryOrder.id` only). **Not** opened when the order is missing from both Accepted lists. If detail fetch fails, drawer still opens with row summary + timeline + localized error + **Retry** — no crash. **Copy reference** works independently of detail fetch success/failure.
...
| **States**      | Browse/listings: loading skeleton, empty results, load error, manual refresh, success/error alerts for create and cancel. **My requests** section: see **Availability request panel states**                                                                                                                                                                                                                                                                                   |
...
- **M4 — Marketplace (Sender → Wayler):** `DeliveryOrder` schema, draft/create/publish/**cancel**, Wayler OPEN feed (filters, sort, Leaflet map previews), **Wayler open orders feed panel states** (loading skeleton, friendly empty, error + Retry, Refresh/Refreshing…; stale cards on error when cached; shared `panel-status-states.tsx`), accept, **ACCEPTED → IN_TRANSIT → DELIVERED** progression, **metadata proof-of-delivery** (submit + read-only Sender view), Wayler accepted panel controls, Sender lifecycle visibility + cancel UI, private `GET /orders/mine`, **Accepted delivery requests panel states** (Sender + Wayler — loading skeleton, friendly empty, error + Retry, Refresh/Refreshing…), **in-app notifications** (schema, API, SDK, order lifecycle dispatch, **chat message dispatch** via `SYSTEM`, bell/dropdown, polling, **order-linked notification click navigation** — mark read, close bell, focus Accepted panel, scroll/highlight, open drawer via **`DeliveryOrder.id`**), **Accepted Order Details drawer** (View details on Sender/Wayler accepted cards — frontend-only; existing `GET /orders/:id`; **Delivery progress timeline**; **loading/error/retry** with accepted-row fallback; **Copy reference** for **`DeliveryOrder.id`**; no route), **order-based chat** (schema, API, SDK, Sender/Wayler Accepted panel UI, modal on `/app`, **10s chat modal polling**), **premium `/app` dashboard UI foundation** (shell, cards, badges, alerts). ✅ (core loop + cancellation + lifecycle + metadata proof + open-feed loading/empty/error/retry + accepted-panel loading/empty/error/retry + notifications + order-linked bell click + accepted-order details drawer + delivery progress timeline + drawer loading/error/retry + drawer Copy reference + chat + chat in-app alerts + chat polling + dashboard visual foundation complete; photo/signature proof, WebSocket/SSE/push/email, **realtime open-order feed**, `CHAT_MESSAGE` type, payment processing/disputes later)
...
- **M7 — Wayler availability & two-sided discovery:** **`WaylerAvailability` + `WaylerAvailabilityRequest` schemas**, migrations `wayler_availability_foundation` + `add_wayler_availability_requests` + `add_delivery_order_source_for_availability_requests`, **`WaylerAvailabilitiesModule` + `WaylerAvailabilityRequestsModule` API + SDK**, **Wayler management UI**, **Sender browse + request UI** (“Request this Wayler”, “My requests to Waylers”, cancel), **Wayler incoming accept/decline UI**, **availability-request panel states** (Sender **My requests** + Wayler **Incoming Sender requests** — loading/empty/error/retry/refresh; shared `panel-status-states.tsx`), **availability-request in-app notifications** (`SYSTEM` on create/decline/cancel; **`ORDER_ACCEPTED`** + **`relatedOrderId`** = **`DeliveryOrder.id`** on accept), **DeliveryOrder conversion on accept** (transactional — `ACCEPTED` order, `sourceType=WAYLER_AVAILABILITY_REQUEST`, `availabilityRequestId`, `deliveryOrderId` in response), **converted-order UI on request panels** (“Converted to order” badge + short order reference; i18n `convertedToOrder` / `linkedOrder` / `orderReference`), **order source badge on Accepted panels** (“From Wayler request” + short request reference; i18n `fromWaylerRequest` / `requestReference`; `delivery-order-source-badge.tsx`), **Accepted-panel auto-refresh after request accept** (`onRequestAccepted` / `onAcceptedOrdersRefresh`), **Accepted delivery requests panel states** (loading/empty/error/retry on converted + posted accepted lists), **converted-order chat** (existing `DeliveryOrder.id` flow — lazy conversation create; Wayler daily-access gate unchanged), **converted-order mock payment** (existing `DeliveryOrder.id` flow — authorize/hold/release; no `sourceType` filter; verified compatible — no code changes), **order-linked notification click** for accept notifications (`ORDER_ACCEPTED` + `relatedOrderId` = `DeliveryOrder.id` → Accepted panel focus/highlight + open drawer), **Accepted Order Details drawer** (View details + **Delivery progress timeline** + **loading/error/retry** + **Copy reference** on converted + posted accepted cards). ✅ (schema + API + SDK + two-sided discovery + request flow + request-panel states + notifications + order conversion + request-panel + accepted-panel source UI + accepted-panel states + refresh + chat + payment + bell click + details drawer + timeline + drawer reliability + Copy reference polish complete). Next: dedicated order detail page/route from bell/badges, public share link, dedicated notification types for create/decline/cancel, auto payment/chat on convert, expiry automation, request detail page, admin moderation, matching, map visualization, availability listing notifications, live carrier tracking on timeline, **realtime availability-request + accepted-order panel updates**, Stripe/real payment, production deployment.
