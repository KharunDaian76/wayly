...
**Safety:** KYC approve/reject and dispute resolve are **controlled metadata mutations** on existing schema fields — no refund, release, capture, payout, order cancel/reassign, or user moderation. Orders, Users, Payments, and System Health panels remain read-only. No secrets (DB URLs, provider keys, payment instruments) in admin responses.
...
- **M12 — Admin Operations Center:** **`AdminOperationsDashboard`** on `/app` for **`ADMIN` / `ARBITRATOR` only**; six panels — **Disputes** (list + **resolve v1**), **KYC** (list + **approve/reject v1**), **Orders**, **Users & Trust/Safety**, **Payments & Escrow**, **System Health & Audit** (last four read-only); SDK `api.admin.*`; loading/error/retry/empty/list UX. ✅ (foundation + KYC approve/reject v1 + dispute resolution v1 — metadata/manual only). Next: payment refund/release/payout, user moderation, persistent audit logs, real provider admin hooks, assign arbitrator, admin realtime, dispute payment execution. See [Admin Operations Center](docs/admin-operations-center.md).
