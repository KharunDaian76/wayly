...
| **Empty**         | When `!loading && !error && orders.length === 0` — **title** (`app.waylerFeed.acceptedPanel.emptyTitle`) + **body** (`app.waylerFeed.acceptedPanel.emptyBody`): accepted work appears after accepting from the **open feed** or **Sender availability requests**. |
...
- **M4 — Marketplace (Sender → Wayler):** `DeliveryOrder` schema, draft/create/publish/**cancel**, Wayler OPEN feed (filters, sort, Leaflet map previews), accept, **ACCEPTED → IN_TRANSIT → DELIVERED** progression, **metadata proof-of-delivery** (submit + read-only Sender view), Wayler accepted panel controls, Sender lifecycle visibility + cancel UI, private `GET /orders/mine`, **Accepted delivery requests panel states** (Sender + Wayler — loading skeleton, friendly empty, error + Retry, Refresh/Refreshing…), **in-app notifications** (schema, API, SDK, order lifecycle dispatch, **chat message dispatch** via `SYSTEM`, bell/dropdown, polling, **order-linked notification click navigation** — mark read, close bell, focus Accepted panel, scroll/highlight, open drawer via **`DeliveryOrder.id`**), **Accepted Order Details drawer** (View details on Sender/Wayler accepted cards — frontend-only; existing `GET /orders/:id`; **Delivery progress timeline**; **loading/error/retry** with accepted-row fallback; **Copy reference** for **`DeliveryOrder.id`**; no route), **order-based chat** (schema, API, SDK, Sender/Wayler Accepted panel UI, modal on `/app`, **10s chat modal polling**), **premium `/app` dashboard UI foundation** (shell, cards, badges, alerts). ✅ (core loop + cancellation + lifecycle + metadata proof + accepted-panel loading/empty/error/retry + notifications + order-linked bell click + accepted-order details drawer + delivery progress timeline + drawer loading/error/retry + drawer Copy reference + chat + chat in-app alerts + chat polling + dashboard visual foundation complete; photo/signature proof, WebSocket/SSE/push/email, `CHAT_MESSAGE` type, payment processing/disputes later)
