...
Both **Sender** (`#sender-accepted-panel`) and **Wayler** (`#wayler-accepted-panel`) **Accepted delivery requests** panels on `/app` share polished loading, empty, error, and refresh UX — frontend-only (`AcceptedPanelEmptyState`, `AcceptedPanelErrorState` in `apps/web/src/app/(app)/app/page.tsx`). **No backend/schema changes.**
...
- **M7 — Wayler availability & two-sided discovery:** **`WaylerAvailability` + `WaylerAvailabilityRequest` schemas**, migrations `wayler_availability_foundation` + `add_wayler_availability_requests` + `add_delivery_order_source_for_availability_requests`, **`WaylerAvailabilitiesModule` + `WaylerAvailabilityRequestsModule` API + SDK**, **Wayler management UI**, **Sender browse + request UI** (“Request this Wayler”, “My requests to Waylers”, cancel), **Wayler incoming accept/decline UI**, **availability-request in-app notifications** (`SYSTEM` on create/decline/cancel; **`ORDER_ACCEPTED`** + **`relatedOrderId`** = **`DeliveryOrder.id`** on accept), **DeliveryOrder conversion on accept** (transactional — `ACCEPTED` order, `sourceType=WAYLER_AVAILABILITY_REQUEST`, `availabilityRequestId`, `deliveryOrderId` in response), **converted-order UI on request panels** (“Converted to order” badge + short order reference; i18n `convertedToOrder` / `linkedOrder` / `orderReference`), **order source badge on Accepted panels** (“From Wayler request” + short request reference; i18n `fromWaylerRequest` / `requestReference`; `delivery-order-source-badge.tsx`), **Accepted-panel auto-refresh after request accept** (`onRequestAccepted` / `onAcceptedOrdersRefresh`), **Accepted delivery requests panel states** (loading/empty/error/retry on converted + posted accepted lists), **converted-order chat** (existing `DeliveryOrder.id` flow — lazy conversation create; Wayler daily-access gate unchanged), **converted-order mock payment** (existing `DeliveryOrder.id` flow — authorize/hold/release; no `sourceType` filter; verified compatible — no code changes), **order-linked notification click** for accept notifications (`ORDER_ACCEPTED` + `relatedOrderId` = `DeliveryOrder.id` → Accepted panel focus/highlight + open drawer), **Accepted Order Details drawer** (View details + **Delivery progress timeline** + **loading/error/retry** + **Copy reference** on converted + posted accepted cards). ✅ (schema + API + SDK + two-sided discovery + request flow + notifications + order conversion + request-panel + accepted-panel source UI + accepted-panel states + refresh + chat + payment + bell click + details drawer + timeline + drawer reliability + Copy reference polish complete). Next: dedicated order detail page/route from bell/badges, public share link, dedicated notification types for create/decline/cancel, auto payment/chat on convert, expiry automation, request detail page, admin moderation, matching, map visualization, availability listing notifications, live carrier tracking on timeline, realtime accepted-order/drawer updates, Stripe/real payment, production deployment.
