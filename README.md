...
- **M12 — Admin Operations Center:** **`AdminOperationsDashboard`** on `/app` for **`ADMIN` / `ARBITRATOR` only**; six panels — **Disputes** (list + **resolve v1**), **KYC** (list + **approve/reject v1**), **Orders**, **Users & Trust/Safety** (list + **suspend/unsuspend v1**, ADMIN-only mutations), **Payments & Escrow**, **System Health & Audit** (health snapshot + **recent admin actions** from audit log); SDK `api.admin.*`; loading/error/retry/empty/list UX; **append-only `AdminAuditLog`** for KYC/dispute/user moderation (best-effort writes). ✅ (foundation + KYC + dispute + audit log + user moderation v1). Next: payment refund/release/payout, user ban/delete, strict transactional audit, real provider admin hooks, assign arbitrator, admin realtime, dispute payment execution. See [Admin Operations Center](docs/admin-operations-center.md).
