...
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| [docs/admin-operations-center.md](docs/admin-operations-center.md) | Read-only Admin / Arbitrator Operations Center — panels, endpoints, access control, security, roadmap |
...
| Admin Operations Center (read-only — disputes, KYC, orders, users, payments, system health on `/app`; `GET /api/v1/admin/*`)                                                                                                                | Complete (read-only foundation)                                                                                                                 |
...
| Admin/arbitrator panel, dispute resolution, payment hold on dispute                                                                                                                                                                         | Read-only admin foundation complete — see [Admin Operations Center](docs/admin-operations-center.md); resolution/mutation workflows not started |
...
- **Read-only admin dispute queue** — `GET /api/v1/admin/disputes` lists disputes for operations monitoring; no resolve/assign API yet
...
Wayly has a **read-only Admin / Arbitrator Operations Center** on `/app` for staff roles only. Normal **`USER`** accounts do not see it.

| Panel                 | Admin API (read-only)                 |
| --------------------- | ------------------------------------- |
| Disputes queue        | `GET /api/v1/admin/disputes`          |
| KYC review queue      | `GET /api/v1/admin/kyc-verifications` |
| Orders monitoring     | `GET /api/v1/admin/orders`            |
| Users & Trust/Safety  | `GET /api/v1/admin/users`             |
| Payments & Escrow     | `GET /api/v1/admin/payments`          |
| System Health & Audit | `GET /api/v1/admin/system-health`     |
...
**Safety:** all panels are list/monitor/snapshot only — no approve/reject, resolve, refund, release, ban, or other mutation actions. No secrets (DB URLs, provider keys, payment instruments) in admin responses.

**Full reference:** [docs/admin-operations-center.md](docs/admin-operations-center.md) — endpoints, security notes, future roadmap, manual test checklist.
...
- **M12 — Admin Operations Center (read-only):** **`AdminOperationsDashboard`** on `/app` for **`ADMIN` / `ARBITRATOR` only**; six read-only panels — **Disputes** (`GET /api/v1/admin/disputes`), **KYC** (`GET /api/v1/admin/kyc-verifications`), **Orders** (`GET /api/v1/admin/orders`), **Users & Trust/Safety** (`GET /api/v1/admin/users`), **Payments & Escrow** (`GET /api/v1/admin/payments`), **System Health & Audit** (`GET /api/v1/admin/system-health`); SDK `api.admin.*`; loading/error/retry/empty/list UX; **no admin mutation actions**. ✅ (read-only foundation complete — safe for demo/staging). Next: KYC approve/reject, dispute resolution, payment refund/release/payout, user moderation, persistent audit logs, real provider admin hooks, admin realtime. See [Admin Operations Center](docs/admin-operations-center.md).
