...
| Admin user moderation v1 (suspend/unsuspend, ADMIN-only, audit logged)                                                                                                                                                                      | Complete (v1)                                                                                                                                                             |
| Admin payment review v1 (metadata-only decisions, ADMIN-only, audit logged; no real refund/release)                                                                                                                                         | Complete (v1)                                                                                                                                                             |
| Admin audit log foundation v1 (`AdminAuditLog` + `GET /api/v1/admin/audit-logs`; append-only KYC/dispute/user moderation audit)                                                                                                             | Complete (v1)                                                                                                                                                             |
...
- **M12 — Admin Operations Center:** **`AdminOperationsDashboard`** on `/app` for **`ADMIN` / `ARBITRATOR` only**; six panels — **Disputes** (list + **resolve v1**), **KYC** (list + **approve/reject v1**), **Orders**, **Users & Trust/Safety** (list + **suspend/unsuspend v1**, ADMIN-only mutations), **Payments & Escrow** (list + **metadata review decisions v1**, ADMIN-only mutations), **System Health & Audit** (health snapshot + **recent admin actions** from audit log); SDK `api.admin.*`; loading/error/retry/empty/list UX; **append-only `AdminAuditLog`** for KYC/dispute/user moderation/payment review (best-effort writes). ✅ (foundation + KYC + dispute + audit log + user moderation + payment review v1). Next: **real** payment refund/release/payout (Stripe), user ban/delete, strict transactional audit, real provider admin hooks, assign arbitrator, admin realtime, dispute payment execution. See [Admin Operations Center](docs/admin-operations-center.md).
