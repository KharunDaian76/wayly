...
Sender **Drafts** panel, filters, maps, and KYC gating are unchanged.
...
**Unchanged card/panel behavior:** **View details** (drawer), **Copy reference** (inside drawer), chat, mock payment, proof, dispute, order-linked notification focus/highlight + drawer open, converted-order source badges, post-accept auto-refresh from availability-request accept, **Sender Published Orders** panel (create/publish/cancel + panel states — see **Sender Published Orders panel states**), **Wayler availability/trips My listings** panel (create/publish/pause/cancel + panel states — see **Wayler availability/trips panel states**), and **Wayler OPEN feed** accept flow (see **Wayler open orders feed panel states**).
...
**Unchanged My listings behavior:** **Create availability** form (draft create), **Publish** / **Pause** / **Cancel** on listing cards, status badges, **Sender browse availability** (`publicList` — Senders see published **ACTIVE** listings), **Sender request-to-Wayler** flow, **Wayler incoming Sender requests** panel, **DeliveryOrder conversion on accept**, **Accepted delivery requests panel refresh**, **Accepted Order Details drawer**, and **order-linked notification click** (focus accepted card + open drawer via **`DeliveryOrder.id`**) — all unchanged.
...
- **M7 — Wayler availability & two-sided discovery:** **`WaylerAvailability` + `WaylerAvailabilityRequest` schemas**, migrations `wayler_availability_foundation` + `add_wayler_availability_requests` + `add_delivery_order_source_for_availability_requests`, **`WaylerAvailabilitiesModule` + `WaylerAvailabilityRequestsModule` API + SDK**, **Wayler management UI** + **Wayler availability/trips panel states** (My listings — loading skeleton, friendly empty, error + Retry, Refresh/Refreshing…; stale listing cards on error when cached; shared `panel-status-states.tsx`), **Sender browse + request UI** (“Request this Wayler”, “My requests to Waylers”, cancel), **Wayler incoming accept/decline UI**, **availability-request panel states** (Sender **My requests** + Wayler **Incoming Sender requests** — loading/empty/error/retry/refresh; shared `panel-status-states.tsx`), **availability-request in-app notifications** (`SYSTEM` on create/decline/cancel; **`ORDER_ACCEPTED`** + **`relatedOrderId`** = **`DeliveryOrder.id`** on accept), **DeliveryOrder conversion on accept** (transactional — `ACCEPTED` order, `sourceType=WAYLER_AVAILABILITY_REQUEST`, `availabilityRequestId`, `deliveryOrderId` in response), **converted-order UI on request panels** (“Converted to order” badge + short order reference; i18n `convertedToOrder` / `linkedOrder` / `orderReference`), **order source badge on Accepted panels** (“From Wayler request” + short request reference; i18n `fromWaylerRequest` / `requestReference`; `delivery-order-source-badge.tsx`), **Accepted-panel auto-refresh after request accept** (`onRequestAccepted` / `onAcceptedOrdersRefresh`), **Accepted delivery requests panel states** (loading/empty/error/retry on converted + posted accepted lists), **converted-order chat** (existing `DeliveryOrder.id` flow — lazy conversation create; Wayler daily-access gate unchanged), **converted-order mock payment** (existing `DeliveryOrder.id` flow — authorize/hold/release; no `sourceType` filter; verified compatible — no code changes), **order-linked notification click** for accept notifications (`ORDER_ACCEPTED` + `relatedOrderId` = `DeliveryOrder.id` → Accepted panel focus/highlight + open drawer), **Accepted Order Details drawer** (View details + **Delivery progress timeline** + **loading/error/retry** + **Copy reference** on converted + posted accepted cards). ✅ (schema + API + SDK + two-sided discovery + Wayler My listings panel states + request flow + request-panel states + notifications + order conversion + request-panel + accepted-panel source UI + accepted-panel states + refresh + chat + payment + bell click + details drawer + timeline + drawer reliability + Copy reference polish complete). Next: dedicated order detail page/route from bell/badges, public share link, dedicated notification types for create/decline/cancel, auto payment/chat on convert, expiry automation, request detail page, admin moderation, matching, map visualization, availability listing notifications, live carrier tracking on timeline, **realtime Wayler availability listing + availability-request + accepted-order panel updates**, Stripe/real payment, production deployment.
