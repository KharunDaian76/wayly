...
| Accepted Order Details drawer (View details; Sender + Wayler; frontend-only; **Delivery progress timeline**; **loading/error/retry**)  | Complete (M4)                   |
...
| Accepted Order Details drawer (View details; Sender + Wayler; frontend-only; **Delivery progress timeline**; **loading/error/retry**) | Complete      |
...
| Panel                                                                          | Behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Wayler Accepted** (`GET /orders/accepted`)                                   | **View details** opens Accepted Order Details drawer (**Delivery progress** timeline; **loading/error/retry** with row fallback); order-linked bell click opens the same drawer after focus/highlight; **Start transit** when `ACCEPTED`; **Mark delivered** when `IN_TRANSIT`; proof form when `IN_TRANSIT` / `DELIVERED`; **Delivered** note when `DELIVERED`; each card has `id="accepted-order-{DeliveryOrder.id}"` for notification focus/highlight; chat/payment/proof actions remain on card; buttons disabled while an action runs |
| **Sender Accepted** (`GET /orders/mine` for ACCEPTED / IN_TRANSIT / DELIVERED) | **View details** opens drawer with **Delivery progress** timeline and **loading/error/retry**; order-linked bell click opens the same drawer after focus/highlight; status badges, contextual notes, read-only proof section when submitted; same `accepted-order-{id}` DOM ids for bell click navigation; chat/payment/proof on card; pending/missing proof notes otherwise                                                                                                                                                               |
...
| Phase       | Behavior                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Loading** | Compact **“Loading order details…”** banner (`app.orders.detailsLoading`, `role="status"`, `aria-live="polite"`) while `GET /orders/:id` is in flight. **Does not blank the drawer** — accepted-row summary (title, status badge, type, role, order reference, source badge, route, reward) and **Delivery progress** timeline stay visible using row/fallback data.                                                                   |
...
| **Error**   | Drawer **does not crash** if detail fetch fails. Shows localized **“Could not load full order details.”** (`app.orders.detailsLoadFailed`) — **no raw API error text**. Shows fallback notice (`app.orders.detailsFallbackNotice`) that the accepted-row summary may be incomplete. Row summary + timeline **remain visible**; detail-only fields stay hidden (`detail?.` guards). **Close**, **Escape**, and **backdrop** still work. |
| **Retry**   | **Retry** button (`app.orders.retryDetails`) re-runs the same `api.orders.detail(order.id)` fetch. Error clears when retry starts; button **disabled while loading**. On success, full details populate and error/fallback notice disappear.                                                                                                                                                                                           |

**Entry points:** identical loading/error/retry behavior when opened from **View details** on an accepted card or from an **order-linked notification** click (after focus/highlight). Notifications **without** `relatedOrderId` still do **not** open the drawer.
...
**i18n** (`app.orders.*`, 8 locales): `viewDetails`, `orderDetails`, `closeDetails`, `orderReference`, `route`, `pickup`, `dropoff`, `reward`, `notes`, `deliveryProgress`, `progressCurrent`, `progressPending`, `dateUnavailable`, `detailsLoading`, `detailsLoadFailed`, `retryDetails`, `detailsFallbackNotice`, plus role/deadline labels; step titles reuse `app.senderPanel.status*` and `app.senderPanel.payment.*` / wayler payment keys.
...
- [ ] **User A** (Sender Accepted) clicks **View details** → drawer opens; compact **Loading order details…** banner may appear briefly; then title, status, route, reward, order reference, **Posted order** label, **Delivery progress** timeline
...
- [ ] **User A** clicks posted-order **`ORDER_ACCEPTED`** notification → card scrolls/highlights; drawer opens with timeline; same loading/error/retry behavior as **View details**
- [ ] **User S** clicks converted availability-request **`ORDER_ACCEPTED`** notification → same (read, close bell, focus, highlight, drawer + timeline; loading/error/retry applies)
...
- **Dedicated order detail page/route** — full `/app/orders/:id` (or similar) with deep links from bell, badges, and notifications (drawer + timeline + **loading/error/retry** exist today; bell already opens drawer after focus)
...
| **Click navigation** | **Implemented for `relatedOrderId`** — marks unread as read, closes popover, switches Sender/Wayler mode, reloads Accepted orders, scrolls to matching card (`accepted-order-{DeliveryOrder.id}`), temporary primary ring highlight (~2.5s), **opens Accepted Order Details drawer** (loading/error/retry with row fallback). **No navigation** when `relatedOrderId` is null — safe display + mark-read only; **no drawer** |
...
6. **Open details drawer** — when the order is found, opens the **Accepted Order Details drawer** (including **Delivery progress** timeline and **loading/error/retry** with accepted-row fallback) for that same **`DeliveryOrder.id`** (same component/state as **View details** on the card). Drawer enriches via existing **`GET /orders/:id`** (`DeliveryOrder.id` only). **Not** opened when the order is missing from both Accepted lists. If detail fetch fails, drawer still opens with row summary + timeline + localized error + **Retry** — no crash.
...
- [ ] **Accepted Order Details drawer opens** for the same **`DeliveryOrder.id`** with **Delivery progress** timeline visible; loading/error/retry same as **View details**
...
- **Dedicated order detail page/route + bell deep link** — full order detail route from notification (today: Accepted panel/card focus + drawer with **Delivery progress** timeline opens from bell click and **View details**; no dedicated route yet)
...
| **Accepted Order Details drawer** (View details; Sender + Wayler; frontend-only; **Delivery progress timeline**; **loading/error/retry**) | Full order detail page/route                                                             |
...
- **No dedicated order detail page/route** — drawer with **Delivery progress** timeline and **loading/error/retry** on accepted cards today; not a full `/app/orders/:id` route
...
- **M4 — Marketplace (Sender → Wayler):** `DeliveryOrder` schema, draft/create/publish/**cancel**, Wayler OPEN feed (filters, sort, Leaflet map previews), accept, **ACCEPTED → IN_TRANSIT → DELIVERED** progression, **metadata proof-of-delivery** (submit + read-only Sender view), Wayler accepted panel controls, Sender lifecycle visibility + cancel UI, private `GET /orders/mine`, **in-app notifications** (schema, API, SDK, order lifecycle dispatch, **chat message dispatch** via `SYSTEM`, bell/dropdown, polling, **order-linked notification click navigation** — mark read, close bell, focus Accepted panel, scroll/highlight, open drawer via **`DeliveryOrder.id`**), **Accepted Order Details drawer** (View details on Sender/Wayler accepted cards — frontend-only; existing `GET /orders/:id`; **Delivery progress timeline**; **loading/error/retry** with accepted-row fallback; no route), **order-based chat** (schema, API, SDK, Sender/Wayler Accepted panel UI, modal on `/app`, **10s chat modal polling**), **premium `/app` dashboard UI foundation** (shell, cards, badges, alerts). ✅ (core loop + cancellation + lifecycle + metadata proof + notifications + order-linked bell click + accepted-order details drawer + delivery progress timeline + drawer loading/error/retry + chat + chat in-app alerts + chat polling + dashboard visual foundation complete; photo/signature proof, WebSocket/SSE/push/email, `CHAT_MESSAGE` type, payment processing/disputes later)
...
- **M7 — Wayler availability & two-sided discovery:** **`WaylerAvailability` + `WaylerAvailabilityRequest` schemas**, migrations `wayler_availability_foundation` + `add_wayler_availability_requests` + `add_delivery_order_source_for_availability_requests`, **`WaylerAvailabilitiesModule` + `WaylerAvailabilityRequestsModule` API + SDK**, **Wayler management UI**, **Sender browse + request UI** (“Request this Wayler”, “My requests to Waylers”, cancel), **Wayler incoming accept/decline UI**, **availability-request in-app notifications** (`SYSTEM` on create/decline/cancel; **`ORDER_ACCEPTED`** + **`relatedOrderId`** = **`DeliveryOrder.id`** on accept), **DeliveryOrder conversion on accept** (transactional — `ACCEPTED` order, `sourceType=WAYLER_AVAILABILITY_REQUEST`, `availabilityRequestId`, `deliveryOrderId` in response), **converted-order UI on request panels** (“Converted to order” badge + short order reference; i18n `convertedToOrder` / `linkedOrder` / `orderReference`), **order source badge on Accepted panels** (“From Wayler request” + short request reference; i18n `fromWaylerRequest` / `requestReference`; `delivery-order-source-badge.tsx`), **Accepted-panel auto-refresh after request accept** (`onRequestAccepted` / `onAcceptedOrdersRefresh`), **converted-order chat** (existing `DeliveryOrder.id` flow — lazy conversation create; Wayler daily-access gate unchanged), **converted-order mock payment** (existing `DeliveryOrder.id` flow — authorize/hold/release; no `sourceType` filter; verified compatible — no code changes), **order-linked notification click** for accept notifications (`ORDER_ACCEPTED` + `relatedOrderId` = `DeliveryOrder.id` → Accepted panel focus/highlight + open drawer), **Accepted Order Details drawer** (View details + **Delivery progress timeline** + **loading/error/retry** on converted + posted accepted cards). ✅ (schema + API + SDK + two-sided discovery + request flow + notifications + order conversion + request-panel + accepted-panel source UI + refresh + chat + payment + bell click + details drawer + timeline + drawer reliability polish complete). Next: dedicated order detail page/route from bell/badges, dedicated notification types for create/decline/cancel, auto payment/chat on convert, expiry automation, request detail page, admin moderation, matching, map visualization, availability listing notifications, live carrier tracking on timeline, realtime detail refresh in drawer, Stripe/real payment, production deployment.
